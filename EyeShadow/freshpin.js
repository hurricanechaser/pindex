...
var FreshPin = {
    constants: {
        ...
        //        upl: 'http://freshpindev.botcodelocal.com/uploaded/',

        //Staging
        //        domain: 'http://photogallery.botcodelocal.com/',
        //        cdn: 'http://photogallery.botcodelocal.com/cdn/',
        //        upl: 'http://photogallery.botcodelocal.com/cdn/content/uploaded/',

        //Prod
        domain: 'http://freshpin.com/',
        cdn: 'http://freshpin.com/cdn/',
        upl: 'http://freshpin.com/cdn/freshpinprod/uploaded/',
        authcookie: 'auth',
        noblock: false,
        infocookie: 'info',
    ...
    unblockUI: function () { this._mask.remove(); },
    appendScriptElement: function (url) { document.write(String.format('<script src="{0}" type="text/javascript"></script>', url)); },
    ...
    tobool: function (s) { return "true" == s; },
    imgMap: function (s, u, attr) { $(s).attr(attr || 'src', this.constants.cdn + u); },
    authenticated: function () { return document.cookie.indexOf(this.constants.authcookie) != -1; },
    visited: function () {
    ...
    attach: function (fn, args, scope, group) {
        ...
        var emits = $.grep(this._emitted, function (item) { return item.group == group; });
    ...
    emit: function (group, scope, args) {
        ...
        var filtered = $.grep(this._funcs, function (it) { return it.group == group; });
    ...
    _returnurl: {},
    setru: function (group, url, force) { if (this._returnurl[group] == undefined || force != undefined) this._returnurl[group] = url; },
...
$(function () {
    var contentpagesflg = $.grep(['about.html', 'copyright.html', 'privacy.html', 'terms.html', 'faq.html', 'help.html', 'pinpractices.html'], function (i) { return $(location).attr('href').toLowerCase().indexOf(i.toLowerCase()) != -1; }).length > 0;
    ...
    $('#query').focus(function () {
        if ($(this).val() == 'Search over 500,000 Pins') {
            $(this).val('');
        }
    });
    $('#query').blur(function () {
        if ($(this).val() == '') {
            $(this).val('Search over 500,000 Pins');
        }
    });

    function search() {
        if (contentpagesflg) FreshPin.constants.noblock = true;
        var q1 = $('#query').val();
        if (q1 != '') {
            params = ['q=' + q1];
    ...
    if (FreshPin.authenticated()) {
        ...
        //                }
        //            } else {
        //                return String.format('You have unsaved points:{0}.You can save these points by signing up for an account.', (function (o) { return o ? o.total : 0; })(FreshPin.readCookieValue('pts')));
        //            }
        //        });
