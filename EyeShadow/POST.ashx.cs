...
namespace EyeShadow
{
    ...
    public class POST : HttpHandlerBase
    {
        ...
        private void Review(HttpContext context)
        {
            ...
            EyeShadow.dbml.Review review = GetEyeShadowContext2.Review.FirstOrDefault(o => o.BIMID == bimid && (o.UserID == Common.UserID || o.SessionID == Common.SessionID));
            if (review == null)
            {
                review = new EyeShadow.dbml.Review();
                review.Answer = answer;
                review.BIMID = bimid;
                review.Question = question;
                review.UserID = Common.UserID;
                review.SessionID = Common.SessionID;
                GetEyeShadowContext2.Review.InsertOnSubmit(review);
                if (Common.UserID.HasValue)
                {
                    GetEyeShadowContext1.Update(new POCOS.AppUser() { ID = Common.UserID.Value, Points = Common.Points }, new[] { "Points" });
                }
            }
            else
            {
            ...
            GetEyeShadowContext2.SubmitChanges();
            var q = from o in GetEyeShadowContext2.Review where (o.UserID == Common.UserID || o.SessionID == Common.SessionID) select o.BIMID;
        ...
        private void DelBoard(HttpContext context)
        {
            ...
            GetEyeShadowContext3.DeleteBoard(boardid).Execute();
        }
        ...
        private void SaveEditBoard(HttpContext context)
        {
            ...
            using (TransactionScope ts = new TransactionScope())
            {
                try
                {
                    ...
                    {
                        GetEyeShadowContext3.DeleteBoardContributor((string)obj["Name"], boardid).Execute();
                    }
                    foreach (JObject obj in bc)
                    {
                        GetEyeShadowContext3.AddBoardContributor((string)obj["Name"], boardid).Execute();
                    }
                    SubSonic.POCOS.Board board = GetEyeShadowContext3.Boards.SingleOrDefault(o => o.ID == boardid);
        ...
        private void DelContributor(HttpContext context)
        {
            ...
            GetEyeShadowContext1.Execute(string.Format(@"DELETE FROM BoardContributor
...
WHERE  (BoardContributor.BoardID = {0}) AND (AppUsers.Name = '{1}')", id, un));
        ...
        private void UserNameAvail(HttpContext context)
        {
            ...
            {
                int un = GetEyeShadowContext1.Single<int>("Select count(*) from AppUsers where Name=@0", _un);
        ...
        {
            int BIMID = int.Parse(context.Request.Params["BIMID"]);
            GetEyeShadowContext3.DeletePin(BIMID, Common.UserID.Value);
        }
        private void ResetPass(HttpContext context)
        {
            string email = context.Request.Params["email"];
            EyeShadow.dbml.AppUsers user = this.GetEyeShadowContext2.AppUsers.FirstOrDefault(o => o.Email == email);
            if (user != null)
            {
        ...
        private void fbLogin(HttpContext context)
        {
            ...
            var db = new PetaPoco.Database(Common.EyeShadowConnectionString, "System.Data.SqlClient");
        ...
        private void AddComment(HttpContext context)
        {
            ...
            EyeShadow.dbml.Comments c = new EyeShadow.dbml.Comments();
            c.Comments1 = comments;
            c.BoardsImagesMappingID = id;
            c.UserID = Common.UserID.Value;
            this.GetEyeShadowContext2.Comments.InsertOnSubmit(c);
        ...
        private void ChangePassword(HttpContext context)
        {
            ...
            {
                EyeShadow.dbml.AppUsers u = this.GetEyeShadowContext2.AppUsers.First(o => o.ID == Common.UserID);
        ...
        private void SaveLike(HttpContext context)
        {
            ...
            if (liked)
            {
                EyeShadow.dbml.Likes l = new EyeShadow.dbml.Likes();
                l.BoardsImagesMappingID = id;
                l.UserID = Common.UserID.Value;
                this.GetEyeShadowContext2.Likes.InsertOnSubmit(l);
                this.GetEyeShadowContext2.SubmitChanges();
            }
            else
            {
                this.GetEyeShadowContext1.Execute(string.Format("Delete From Likes where BoardsImagesMappingID={0}", id));
            }
        }
        ...
        private void SaveCreatedBoard(HttpContext context)
        {
            ...
            string name = context.Request.Params["name"];
            EyeShadow.dbml.Boards b = this.GetEyeShadowContext2.Boards.FirstOrDefault(o => o.Name == name && o.UserID == Common.UserID);
            if (b != null)
        ...
        private void SaveBoard(HttpContext context)
        {
            ...
            EyeShadow.dbml.Boards board = new EyeShadow.dbml.Boards()
            {
                Name = name,
                CatID = cat,
                UserID = Common.UserID
            };
            this.GetEyeShadowContext2.Boards.InsertOnSubmit(board);
            this.GetEyeShadowContext2.SubmitChanges();
            if (!string.IsNullOrEmpty(contributors))
            {
                ...
                {
                    this.GetEyeShadowContext2.BoardContributor.InsertOnSubmit(new EyeShadow.dbml.BoardContributor()
                    {
                        BoardID = board.ID,
                        ContributorID = GetEyeShadowContext3.AppUsers.First(o => o.Email == contributor).ID
                    });
                }
                this.GetEyeShadowContext2.SubmitChanges();
            }
        ...
        private void ValidateContributor(HttpContext context)
        {
            ...
            EyeShadow.dbml.AppUsers user = this.GetEyeShadowContext2.AppUsers.FirstOrDefault(o => o.Email == contributor || o.Name == contributor);
            if (user != null)
        ...
        private void RequestInvite(HttpContext context)
        {
            ...
            if (user == null)
            {
                user = new EyeShadow.dbml.AppUsers();
                user.Email = email;
                ...
                client.Send(mess);
                user.Invite = hash;
                this.GetEyeShadowContext2.AppUsers.InsertOnSubmit(user);
                this.GetEyeShadowContext2.SubmitChanges();
        ...
        {
            EyeShadow.dbml.Boards boards = new EyeShadow.dbml.Boards();
            boards.Name = context.Request.Params["name"];
            this.GetEyeShadowContext2.Boards.InsertOnSubmit(boards);
            this.GetEyeShadowContext2.SubmitChanges();
        }
        private void Board(HttpContext context)
        {
            using (TransactionScope scope = new TransactionScope())
            {
                try
                {
                    ...
                    if (_checked)
                    {
                        this.GetEyeShadowContext2.BoardsImagesMapping.InsertOnSubmit(new EyeShadow.dbml.BoardsImagesMapping()
                        {
                            ImageID = imageId,
                            BoardID = catid,
                            UserID = Common.UserID
                    ...
                    {
                        var query = this.GetEyeShadowContext2.BoardsImagesMapping.Where(o => o.BoardID == catid && o.ImageID == imageId);
                        this.GetEyeShadowContext2.BoardsImagesMapping.DeleteAllOnSubmit(query);

                    }
                    this.GetEyeShadowContext2.SubmitChanges();
                    SqlHelper.ExecuteNonQuery((SqlConnection)((EntityConnection)this.GetEyeShadowContext2.Connection).StoreConnection, "dbo.SetTaggedFlag", imageId);
                    scope.Complete();
                }
                catch (Exception e)
                {
        ...
        private void Login(HttpContext context)
        {
            string user = context.Request.Params["user"];
            string pass = context.Request.Params["pass"];
            EyeShadow.dbml.User obj = this.GetEyeShadowContext2.User.FirstOrDefault(o => o.Name == user && o.Password == pass);
            if (obj != null)
            {
        ...
        private void Cat(HttpContext context)
        {
            using (TransactionScope scope = new TransactionScope())
            {
                try
                {
                    ...
                    if (_checked)
                    {
                        this.GetEyeShadowContext2.CategoryImagesMapping.InsertOnSubmit(new EyeShadow.dbml.CategoryImagesMapping()
                        {
                            ImageID = imageId,
                            CategoryID = catid,
                            UserID = Common.UserID
                    ...
                    {
                        var query = this.GetEyeShadowContext2.CategoryImagesMapping.Where(o => o.CategoryID == catid && o.ImageID == imageId);
                        this.GetEyeShadowContext2.CategoryImagesMapping.DeleteAllOnSubmit(query);
