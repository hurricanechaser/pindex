...
namespace HairStyle
{
    ...
    public class POST : HttpHandlerBase
    {
        ...
        private void Review(HttpContext context)
        {
            ...
            HairStyle.dbml.Review review = GetHairStyleContext2.Review.FirstOrDefault(o => o.BIMID == bimid && (o.UserID == Common.UserID || o.SessionID == Common.SessionID));
            if (review == null)
            {
                review = new HairStyle.dbml.Review();
                review.Answer = answer;
                review.BIMID = bimid;
                review.Question = question;
                review.UserID = Common.UserID;
                review.SessionID = Common.SessionID;
                GetHairStyleContext2.Review.InsertOnSubmit(review);
                if (Common.UserID.HasValue)
                {
                    GetHairStyleContext1.Update(new POCOS.AppUser() { ID = Common.UserID.Value, Points = Common.Points }, new[] { "Points" });
                }
            }
            else
            {
            ...
            var q = from o in GetHairStyleContext2.Review where (o.UserID == Common.UserID || o.SessionID == Common.SessionID) select o.BIMID;
        ...
        private void DelContributor(HttpContext context)
        {
            ...
            GetHairStyleContext1.Execute(string.Format(@"DELETE FROM BoardContributor
...
WHERE  (BoardContributor.BoardID = {0}) AND (AppUsers.Name = '{1}')", id, un));
        ...
        private void UserNameAvail(HttpContext context)
        {
            ...
            {
                int un = GetHairStyleContext1.Single<int>("Select count(*) from AppUsers where Name=@0", _un);
        ...
        private void ResetPass(HttpContext context)
        {
            ...
            HairStyle.dbml.AppUsers user = this.GetHairStyleContext2.AppUsers.FirstOrDefault(o => o.Email == email);
            if (user != null)
            {
        ...
        private void AddComment(HttpContext context)
        {
            ...
            HairStyle.dbml.Comments c = new HairStyle.dbml.Comments();
            c.Comments1 = comments;
            c.BoardsImagesMappingID = id;
            c.UserID = Common.UserID.Value;
            this.GetHairStyleContext2.Comments.InsertOnSubmit(c);
            this.GetHairStyleContext2.SubmitChanges();
        }
        ...
        private void SaveLike(HttpContext context)
        {
            ...
            if (liked)
            {
                HairStyle.dbml.Likes l = new HairStyle.dbml.Likes();
                l.BoardsImagesMappingID = id;
                l.UserID = Common.UserID.Value;
                this.GetHairStyleContext2.Likes.InsertOnSubmit(l);
                this.GetHairStyleContext2.SubmitChanges();
            }
            else
            {
                this.GetHairStyleContext1.Execute(string.Format("Delete From Likes where BoardsImagesMappingID={0}", id));
            }
        }
        ...
        private void SaveRepin(HttpContext context)
        {
            ...
            HairStyle.dbml.BoardsImagesMapping bim = new HairStyle.dbml.BoardsImagesMapping();
            bim.ImageID = id;
            bim.Source = source;
            bim.Image_Title = desc;
            bim.UserID = Common.UserID;
            bim.BoardID = board;
            this.GetHairStyleContext2.BoardsImagesMapping.InsertOnSubmit(bim);
            this.GetHairStyleContext2.SubmitChanges();
        }
        ...
        {
            string name = context.Request.Params["name"];
            HairStyle.dbml.Boards b = this.GetHairStyleContext2.Boards.FirstOrDefault(o => o.Name == name && o.UserID == Common.UserID);
            if (b != null)
        ...
        private void SaveBoard(HttpContext context)
        {
            ...
            HairStyle.dbml.Boards board = new HairStyle.dbml.Boards()
            {
                Name = name,
                CatID = cat,
                UserID = Common.UserID
            };
            this.GetHairStyleContext2.Boards.InsertOnSubmit(board);
            this.GetHairStyleContext2.SubmitChanges();
            if (!string.IsNullOrEmpty(contributors))
            {
                ...
                {
                    this.GetHairStyleContext2.BoardContributor.InsertOnSubmit(new HairStyle.dbml.BoardContributor()
                    {
                        BoardID = board.ID,
                        ContributorID = GetHairStyleContext3.AppUsers.First(o => o.Email == contributor).ID
                    });
                }
                this.GetHairStyleContext2.SubmitChanges();
            }
        ...
        private void ValidateContributor(HttpContext context)
        {
            ...
            HairStyle.dbml.AppUsers user = this.GetHairStyleContext2.AppUsers.FirstOrDefault(o => o.Email == contributor || o.Name == contributor);
            if (user != null)
        ...
        private void SaveAddedPin(HttpContext context)
        {
            ...
            using (TransactionScope scope = new TransactionScope())
            {
                try
                {
                    HairStyle.dbml.Images i = new HairStyle.dbml.Images();
                    i.CRC64 = _crc64;
                    i.MURMUR2 = _murmur2;
                    i.Tagged = true;
                    i.FNV1a = _fnv1a;
                    ...
                    HairStyle.dbml.HairStyleDataContext _context = this.GetHairStyleContext2;
        ...
        private void SaveUploadedPin(HttpContext context)
        {
            ...
            using (TransactionScope scope = new TransactionScope())
            {
                try
                {
                    ...
                    i.Tagged = true;
                    i.RelativeImage_Path = relPath;
                    this.GetHairStyleContext2.Images.InsertOnSubmit(i);
                    this.GetHairStyleContext2.SubmitChanges();
                    HairStyle.dbml.BoardsImagesMapping bim = new HairStyle.dbml.BoardsImagesMapping();
                    bim.BoardID = board;
                    bim.ImageID = i.ID;
        ...
        private void RequestInvite(HttpContext context)
        {
            ...
            if (user == null)
            {
                user = new HairStyle.dbml.AppUsers();
                user.Email = email;
                ...
                client.Send(mess);
                user.Invite = hash;
                this.GetHairStyleContext2.AppUsers.InsertOnSubmit(user);
                this.GetHairStyleContext2.SubmitChanges();
        ...
        {
            HairStyle.dbml.Boards boards = new HairStyle.dbml.Boards();
            boards.Name = context.Request.Params["name"];
            this.GetHairStyleContext2.Boards.InsertOnSubmit(boards);
            this.GetHairStyleContext2.SubmitChanges();
        }
        private void Board(HttpContext context)
        {
            using (TransactionScope scope = new TransactionScope())
            {
                try
                {
                    ...
                    if (_checked)
                    {
                        this.GetHairStyleContext2.BoardsImagesMapping.InsertOnSubmit(new HairStyle.dbml.BoardsImagesMapping()
                        {
                            ImageID = imageId,
                            BoardID = catid,
                            UserID = Common.UserID
                    ...
                    {
                        var query = this.GetHairStyleContext2.BoardsImagesMapping.Where(o => o.BoardID == catid && o.ImageID == imageId);
                        this.GetHairStyleContext2.BoardsImagesMapping.DeleteAllOnSubmit(query);

                    }
                    this.GetHairStyleContext2.SubmitChanges();
                    SqlHelper.ExecuteNonQuery((SqlConnection)((EntityConnection)this.GetHairStyleContext2.Connection).StoreConnection, "dbo.SetTaggedFlag", imageId);
                    scope.Complete();
                }
                catch (Exception e)
                {
        ...
        private void Login(HttpContext context)
        {
            string user = context.Request.Params["user"];
            string pass = context.Request.Params["pass"];
            HairStyle.dbml.User obj = this.GetHairStyleContext2.User.FirstOrDefault(o => o.Name == user && o.Password == pass);
            if (obj != null)
            {
        ...
        private void Cat(HttpContext context)
        {
            using (TransactionScope scope = new TransactionScope())
            {
                try
                {
                    ...
                    if (_checked)
                    {
                        this.GetHairStyleContext2.CategoryImagesMapping.InsertOnSubmit(new HairStyle.dbml.CategoryImagesMapping()
                        {
                            ImageID = imageId,
                            CategoryID = catid,
                            UserID = Common.UserID
                    ...
                    {
                        var query = this.GetHairStyleContext2.CategoryImagesMapping.Where(o => o.CategoryID == catid && o.ImageID == imageId);
                        this.GetHairStyleContext2.CategoryImagesMapping.DeleteAllOnSubmit(query);
