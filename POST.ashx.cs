...
namespace PindexProd
{
    ...
    public class POST : HttpHandlerBase
    {
        ...
        private void DelBoard(HttpContext context)
        {
            ...
            GetPindexProdContext3.DeleteBoard(boardid).Execute();
        }
        ...
        private void SaveEditBoard(HttpContext context)
        {
            ...
            using (TransactionScope ts = new TransactionScope())
            {
                try
                {
                    ...
                    {
                        GetPindexProdContext3.DeleteBoardContributor((string)obj["Name"], boardid).Execute();
                    }
                    foreach (JObject obj in bc)
                    {
                        GetPindexProdContext3.AddBoardContributor((string)obj["Name"], boardid).Execute();
                    }
                    SubSonic.POCOS.Board board = GetPindexProdContext3.Boards.SingleOrDefault(o => o.ID == boardid);
        ...
        private void DelContributor(HttpContext context)
        {
            ...
            GetPindexProdContext1.Execute(string.Format(@"DELETE FROM BoardContributor
...
WHERE  (BoardContributor.BoardID = {0}) AND (AppUsers.Name = '{1}')", id, un));
        ...
        private void UserNameAvail(HttpContext context)
        {
            ...
            {
                int un = GetPindexProdContext1.Single<int>("Select count(*) from AppUsers where Name=@0", _un);
        ...
        {
            int BIMID = int.Parse(context.Request.Params["BIMID"]);
            GetPindexProdContext3.DeletePin(BIMID, Common.UserID.Value);
        }
        private void ResetPass(HttpContext context)
        {
            ...
            if (user != null)
            {
                ...
                MailMessage mess = new MailMessage("support@freshpin.com", email);
        ...
        private void fbLogin(HttpContext context)
        {
            ...
            var db = new PetaPoco.Database(Common.PindexProdConnectionString, "System.Data.SqlClient");
        ...
        private void SaveRepin(HttpContext context)
        {
            ...
            PindexProd.dbml.BoardsImagesMapping bim = new PindexProd.dbml.BoardsImagesMapping();
            bim.ImageID = id;
            bim.Source = source;
            bim.Image_Title = desc;
            bim.UserID = Common.UserID;
            bim.BoardID = board;
            this.GetPindexProdContext2.BoardsImagesMapping.InsertOnSubmit(bim);
            this.GetPindexProdContext2.SubmitChanges();
        }
        ...
        private void SaveCreatedBoard(HttpContext context)
        {
            ...
            string name = context.Request.Params["name"];
            PindexProd.dbml.Boards b = this.GetPindexProdContext2.Boards.FirstOrDefault(o => o.Name == name && o.UserID == Common.UserID);
            if (b != null)
        ...
        private void SaveBoard(HttpContext context)
        {
            ...
            PindexProd.dbml.Boards board = new PindexProd.dbml.Boards()
            {
                Name = name,
                CatID = cat,
                UserID = Common.UserID
            };
            this.GetPindexProdContext2.Boards.InsertOnSubmit(board);
            this.GetPindexProdContext2.SubmitChanges();
            if (!string.IsNullOrEmpty(contributors))
            {
                ...
                {
                    this.GetPindexProdContext2.BoardContributor.InsertOnSubmit(new PindexProd.dbml.BoardContributor()
                    {
                        BoardID = board.ID,
                        ContributorID = GetPindexProdContext3.AppUsers.First(o => o.Email == contributor).ID
                    });
                }
                this.GetPindexProdContext2.SubmitChanges();
            }
        ...
        private void SaveAddedPin(HttpContext context)
        {
            ...
            using (TransactionScope scope = new TransactionScope())
            {
                try
                {
                    PindexProd.dbml.Images i = new PindexProd.dbml.Images();
                    i.CRC64 = _crc64;
                    i.MURMUR2 = _murmur2;
                    i.Tagged = true;
                    i.FNV1a = _fnv1a;
                    ...
                    PindexProd.dbml.PindexProdDataContext _context = this.GetPindexProdContext2;
        ...
        private void SaveUploadedPin(HttpContext context)
        {
            ...
            using (TransactionScope scope = new TransactionScope())
            {
                try
                {
                    ...
                    i.Tagged = true;
                    i.RelativeImage_Path = relPath;
                    this.GetPindexProdContext2.Images.InsertOnSubmit(i);
                    this.GetPindexProdContext2.SubmitChanges();
                    PindexProd.dbml.BoardsImagesMapping bim = new PindexProd.dbml.BoardsImagesMapping();
                    bim.BoardID = board;
                    bim.ImageID = i.ID;
        ...
        private void AppLogin(HttpContext context)
        {
            ...
            var obj = (from o in GetPindexProdContext2.AppUsers
            ...
            else
            {
                ...
                GetPindexProdContext3.UpdatePoints(obj.ID, Common.SessionID).Execute();
                JObject jobj = JObject.Parse(context.Server.UrlDecode(CookieUtil.ReadCookie(Common.sessioncookie)));
                int? points = (from o in GetPindexProdContext4.AppUsers where o.ID == obj.ID select o.Points).First();
                var ids = (from o in GetPindexProdContext4.Reviews where o.ID == obj.ID select o.BIMID);
        ...
        {
            PindexProd.dbml.Boards boards = new PindexProd.dbml.Boards();
            boards.Name = context.Request.Params["name"];
            this.GetPindexProdContext2.Boards.InsertOnSubmit(boards);
            this.GetPindexProdContext2.SubmitChanges();
        }
        private void Board(HttpContext context)
        {
            using (TransactionScope scope = new TransactionScope())
            {
                try
                {
                    ...
                    if (_checked)
                    {
                        this.GetPindexProdContext2.BoardsImagesMapping.InsertOnSubmit(new PindexProd.dbml.BoardsImagesMapping()
                        {
                            ImageID = imageId,
                            BoardID = catid,
                            UserID = Common.UserID
                    ...
                    {
                        var query = this.GetPindexProdContext2.BoardsImagesMapping.Where(o => o.BoardID == catid && o.ImageID == imageId);
                        this.GetPindexProdContext2.BoardsImagesMapping.DeleteAllOnSubmit(query);

                    }
                    this.GetPindexProdContext2.SubmitChanges();
                    SqlHelper.ExecuteNonQuery((SqlConnection)((EntityConnection)this.GetPindexProdContext2.Connection).StoreConnection, "dbo.SetTaggedFlag", imageId);
                    scope.Complete();
                }
                catch (Exception e)
                {
        ...
        private void Login(HttpContext context)
        {
            string user = context.Request.Params["user"];
            string pass = context.Request.Params["pass"];
            PindexProd.dbml.User obj = this.GetPindexProdContext2.User.FirstOrDefault(o => o.Name == user && o.Password == pass);
            if (obj != null)
            {
        ...
        private void Cat(HttpContext context)
        {
            using (TransactionScope scope = new TransactionScope())
            {
                try
                {
                    ...
                    if (_checked)
                    {
                        this.GetPindexProdContext2.CategoryImagesMapping.InsertOnSubmit(new PindexProd.dbml.CategoryImagesMapping()
                        {
                            ImageID = imageId,
                            CategoryID = catid,
                            UserID = Common.UserID
                    ...
                    {
                        var query = this.GetPindexProdContext2.CategoryImagesMapping.Where(o => o.CategoryID == catid && o.ImageID == imageId);
                        this.GetPindexProdContext2.CategoryImagesMapping.DeleteAllOnSubmit(query);
