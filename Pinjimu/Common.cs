...
namespace Pinjimu
{
    public static partial class Common
    {
        ...
        public static Data.Standalone.AppUsers User
        {
            get
            {
                if (UserID.HasValue)
                {
                    Data.Standalone.AppUsers user = context.Items["user"] as Data.Standalone.AppUsers;
                    if (user == null)
                    {
                        SqlConnection conn = new SqlConnection(DataConnectionString);
                        conn.Open();
                        context.Items["user"] = user = conn.Get<Data.Standalone.AppUsers>(UserID);
                    }
                    return user;
                }
        ...
        public static Data.Standalone.AppUsers VUser
        {
            get
            {
                if (VUserID.HasValue)
                {
                    Data.Standalone.AppUsers vuser = context.Items["vuser"] as Data.Standalone.AppUsers;
                    if (vuser == null)
                    {
                        SqlConnection conn = new SqlConnection(DataConnectionString);
                        conn.Open();
                        context.Items["vuser"] = vuser = conn.Get<Data.Standalone.AppUsers>(VUserID);
                    }
                    return vuser;
                }
                throw new InvalidOperationException("Visited UserID not available");
            }
        }
