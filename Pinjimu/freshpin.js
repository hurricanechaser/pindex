...
var FreshPin = {
    ...
    _scrollPos: null,
    pushScrollPos: function (v) {
        if (!FreshPin._scrollPos) FreshPin._scrollPos = new Hashtable();
        var tmp = FreshPin._scrollPos;
        var k = ho();
        tmp.put(k, v);
    },
    getScrollPos: function () {
        if (FreshPin._scrollPos) {
            return FreshPin._scrollPos
                .get(ho());
        } else return null;
    },
    remState: function (o) {
...
$(function () {
    $.ajaxSetup({
        ...
        complete: function () {
            FreshPin.unblockUI();
            if (FreshPin.loaduserdetails) FreshPin.loaduserdetails();
        }
    });
    //$('#sautocomplete').css({ left: $('#query').offset().left });
    var ph = $('input:text[placeholder], textarea[placeholder]')
    if (ph.placeholder) ph.placeholder();
