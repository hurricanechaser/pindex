...
WHERE  (BoardContributor.BoardID = {0}) AND (AppUsers.Name = '{1}')", id, un));
        ...
        {
            string un = context.Request.Params["un"];
            if (GetDataContext2.AppUsers.Any(o => o.Name == un)) context.Response.WriteError(string.Format(@"{0} not available", un));
            else context.Response.Write(string.Format(@"{0} is available", un));
