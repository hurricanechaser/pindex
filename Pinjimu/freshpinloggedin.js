...
function FollowUnFollowUser(f_ID, f_img, f_txt) {
    if (FreshPin.authenticated()) {
        ...
        FreshPin.remState(ho('followings'));
...
$(function () {
    ...
    function load() {
        if (h() == 'boards') {
            $.each(FreshPin.constants.data, function (ind, item) {
                ...
                while (l++ < 5) item.images.push({ url: FreshPin.constants.cdn + 'img/paper.jpg', ID: null, height: 232, width: 232 });
            });
            set();
            ...
            if (FreshPin.constants.data && FreshPin.constants.preventReload) {
                ...
                $.getJSON('GET?t=getboards', function (dt) {
                    $.each(dt, function (ind, item) {
                        var l = item.images.length;
                        while (l++ < 5) item.images.push({ ID: null, url: FreshPin.constants.cdn + 'img/paper.jpg', height: 232, width: 232 });
                    });
    ...
    ////////////////////////////end of authenticated and not visited///////////////////////////////
    ////////////////////////////visited and not authenticated///////////////////////////////
    if (!FreshPin.authenticated() && FreshPin.visited()) {
    }
    ////////////////////////////Authenticated///////////////////////////////
    if (FreshPin.authenticated()) {
        ...
        FreshPin.loaduserdetails = function () {
            if (!FreshPin.visited()) {
                var un = FreshPin.getUN();
                switch (FreshPin.getCVal('speciality', 'info')) {
                    case 'M':
                        $('img[name="cSymbol"]').css({ display: 'inline' }).attr('src', 'img/M-Icon.jpg');
                        $('#profileWr').removeClass().addClass('profile-info-FurnManu');
                        $('#cLogo').css({ display: 'inline' }).attr('src', 'img/prof-FurnManu-icon.jpg');
                        break
                    case 'C':
                        $('img[name="cSymbol"]').css({ display: 'inline' }).attr('src', 'img/C-Icon.jpg');
                        $('#profileWr').removeClass().addClass('profile-info-Contractor');
                        $('#cLogo').css({ display: 'inline' }).attr('src', 'img/prof-Contractor-icon.jpg');
                        break;
                    case 'D':
                        $('img[name="cSymbol"]').css({ display: 'inline' }).attr('src', 'img/D-Icon.jpg');
                        $('#profileWr').removeClass().addClass('profile-info-designer');
                        $('#cLogo').css({ display: 'inline' }).attr('src', 'img/prof-designer-icon.jpg');
                        break;
                    case null:
                        $('img[name="cSymbol"],#cLogo').css({ display: 'none' }).attr('src', '');
                        $('#profileWr').removeClass().addClass('profile-info');
                        break;
                }
                $('#username').text(un);
                $('#p_name').text(un);
        ...
        .jqm({
            ...
            onHide: function (hash) {
                ...
                $('#uplPDesc').val('');
                $('#uplPCharCount').html(FreshPin.constants.charCount);
                $('#uploadedImage').attr('src', FreshPin.constants.cdn + 'img/pinjimu/default_thumbnail.jpg');
        ...
        .jqm({
            ...
            onHide: function (hash) {
                ...
                $('#urlimages').attr('src', FreshPin.constants.cdn + 'img/pinjimu/default_thumbnail.jpg');
            }
        });
        ...
        .jqm({
            ...
            onHide: function (hash) {
                ...
                if ($('#pin').css('display') == 'none')
                    ...
                    hash.o.css('z-index', parseInt($('#pin').css('z-index')) - 1);
                hash.w.css({ top: 0 }).hide();
                $('#RPDesc').val('');
        ...
        $('#editpin').css(cssCenterY('#editpin'))
        .jqm({
            overlay: 100,
            onShow: function (hash) {
                hash.o.show().transition({
        ...
        $('#comment').css(cssCenterY('#comment'))
        .jqm({
            overlay: 100,
            onShow: function (hash) {
                hash.o.show().transition({
        ...
        $("#likepint").click(function () {
            ...
            var id = o.BIMID;
            var me = this;
            $.post('POST?t=savelike', { id: id, liked: !liked }, function () {
                o.liked = !liked;
        ...
        $('#FindImages').click(function () {
            ...
            if (imageValidate(url)) {
                ...
                $.get('GET', { t: 'getimagesinurl', url: url }, function (data) {
                    $('#lmfindimages').hide();
                    var l = data.length, i = 0, setimg = true;
                    ...
                    $.imgpreload(data, {
                        each: function () {
                            var me = $(this);
                            var src = me.attr('src');
                            if (me.prop('width') > 200 && me.prop('height') > 200) {
                                urls.push({ height: this.height, width: this.width, src: src });
                                if (setimg) {
                                    $("#urlimages").attr('src', src).attr('index', imgActive);
                                    setimg = false;
                                }
                            }
                            $('#lmpreloadimages span').html('Loading Images ' + (++i) + ' Of ' + l);
                        },
                        all: function () {
                            $('#lmpreloadimages').hide();
                        }
                    });
                }, 'json');
            }
        ...
        var zoomed = false;
        $('#prev,#prevz').click(function () {
            if (urls && urls.length > 0)
                if (imgActive != 0) {
                    var img = urls[--imgActive];
                    $('#urlimages').attr('src', img.src).attr('index', imgActive);
                    if (zoomed) {
                        var winwr = $(window);
                        var width = winwr.width();
                        var height = winwr.height();
                        var ih = img.height > height ? ((img.height - (img.height - height)) - 100) : img.height, iw = img.width > width ? ((img.width - (img.width - width)) - 100) : img.width;
                        ...
                        $('#ZoomBox').transition({
                            ...
                            height: ih,
                            width: iw
                        }, 'fast', function () { $('#prevz,#nextz').css({ top: (ih / 2) - 36 }); zoomed = true; });
                    }
                }
        });
        $('div[name="navCont"]').hover(function () {
            $(this).find('span').show();
        }, function () {
            $(this).find('span').hide();
        });
        $('#next,#nextz').click(function () {
            if (urls && urls.length > 0)
                if (imgActive != (urls.length - 1)) {
                    var img = urls[++imgActive];
                    $("#urlimages").attr('src', img.src).attr('index', imgActive);
                    if (zoomed) {
                        var winwr = $(window);
        ...
        $('#urlimages,#zoomt').click(function () {
            if (urls && urls.length > 0) {
                var me = $('#urlimages');
                var img = urls[parseInt(me.attr('index'))];
                var winwr = $(window);
                var width = winwr.width();
                var height = winwr.height();
                var pos = me.offset();
                ...
                $('#ZoomBox').css({ top: pos.top, left: pos.left, width: me.width(), height: me.height(), display: 'block' }).transition({
                    left: width / 2 - iw / 2,
                    top: height / 2 - ih / 2,
                    height: ih,
                    width: iw
        ...
        $('#ZoomClose').click(zoomclose);
        function imageValidate(img, opt) {
            opt = opt || 0;
            var default_loc = FreshPin.constants.cdn + 'img/pinjimu/default_thumbnail.jpg';
...
FreshPin.attach(function () {
    ...
    $(String.format("#speciality input:checkbox[value='{0}']", FreshPin.constants.userSpeciality)).attr('checked', true);
    ...
    $('#changepasswordt').click(function () {
        FreshPin.closeall();
        $('#changepassword').jqmShow();
    });
    $('#changepassword').css(cssCenterY('#changepassword')).jqm({
        overlay: 100,
        onShow: function (hash) {
            hash.o.show().transition({
                opacity: .5
    ...
    $('#sp').click(function () {
        ...
        if (name == '') {
            ...
            $.post('POST?t=saveprofile', { name: name, fn: fn, first_name: first_name, about: about, location: loc, website: website, speciality: speciality }, function (dt, res, opts) {
                //$(location).attr('hash', FreshPin.getru('settings'));
                $('#p_about').text(about);
                set();
                FreshPin._state.clear();
                location.hash = '#boards';
            }, 'text').error(function (res) {
                alert(res.responseText);
            });
