...
var FreshPin = {
    constants: {
        ...
        //        cdn: 'http://photogallery.botcodelocal.com/nails/cdn/',
...
$(function () {
    ...
    $('#query_button').off();
    $('#query').focus(function () { if ($(this).val() == '查寻500,000个别针') { $(this).val(''); } });
    $('#query').blur(function () { if ($(this).val() == '') { $(this).val('查寻500,000个别针'); } });
    function search() {
        var q1 = $('#query').val();
        if (q1 != '') {
            params = ['q=' + q1];
    ...
    if (FreshPin.authenticated()) {
        $('.logininfo').html('<a class="nav" style="padding: 20px 27px 11px 40px;"><span '
                       ...
                       + ' <span></span></a>'
                       + ' <ul>'
                       + String.format('<li><a href="{0}#boards">議會</a></li>', pre)
                        + String.format('<li class="beforeDivider"><a href="{0}#filter=pins">銷</a></li>', pre)
                        + String.format('<li class="divider"><a href="{0}#filter=likes">喜歡</a></li>', pre)
                         + String.format('<li><a href="{0}#settings">設置</a></li>', pre)
                         + '<li><a id="logout" href="javascript:void(0)">註銷</a></li>'
                       + ' </ul>');
        ...
        $('.logininfo').html('<a href="login" class="nav">登錄</a>');
    }
